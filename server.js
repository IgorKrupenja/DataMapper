...
import { generateMessageTable } from "./js/convert/pdf.js";
...
app.post(
  ...
  (req, res) => {
    ...
    const html = generateMessageTable(
      template,
      messages,
    ...
    try {
      res.json({ response: convertHtmlToPdf(html) });
    } catch (error) {
      res.status(500).send("Error generating PDF from template");
    }
  }
);
